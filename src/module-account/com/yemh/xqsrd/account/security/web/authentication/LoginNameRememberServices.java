...
public class LoginNameRememberServices extends AbstractRememberMeServices {

    public LoginNameRememberServices(String key, UserDetailsService userDetailsService) {
    ...
    protected UserDetails processAutoLoginCookie(String[] cookieTokens,
        ...
        String expectedTokenSignature = makeTokenSignature(tokenExpiryTime,
                ((XUser)userDetails).getLoginName(), userDetails.getPassword());
     ...
     */
    protected String makeTokenSignature(long tokenExpiryTime, String loginName,
            String password) {
        String data = loginName + ":" + tokenExpiryTime + ":" + password + ":" + getKey();
    ...
    public void onLoginSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        String loginName = retrieveLoginName(successfulAuthentication);
        String password = retrievePassword(successfulAuthentication);

        // If unable to find a loginName and password, just abort as
        // LoginNameRememberServices is
        // unable to construct a valid token in this case.
        if (!StringUtils.hasLength(loginName)) {
            logger.debug("Unable to retrieve loginName");
            return;
        }

        if (!StringUtils.hasLength(password)) {
            UserDetails user = getUserDetailsService().loadUserByUsername(loginName);
            password = user.getPassword();

            if (!StringUtils.hasLength(password)) {
                logger.debug("Unable to obtain password for user: " + loginName);
                return;
            }
        }
        ...
        String signatureValue = makeTokenSignature(expiryTime, loginName, password);

        setCookie(new String[] { loginName, Long.toString(expiryTime), signatureValue },
                tokenLifetime, request, response);

        if (logger.isDebugEnabled()) {
            logger.debug("Added remember-me cookie for user '" + loginName
    ...
    protected String retrieveLoginName(Authentication authentication) {
        if (isInstanceOfUserDetails(authentication)) {
            return ((XUser) authentication.getPrincipal()).getLoginName();
        }
        else {
